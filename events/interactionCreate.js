...
async function handleJoinMessagePanel(interaction) {
  ...
  if (interaction.isButton() && interaction.customId === "joinmsg_toggle") {
    ...
    const next = setGuildJoinSetting(guildId, { ...setting, enabled: !setting.enabled });

    return interaction.update({
      embeds: [settingpanel.buildPanel(next, getGuildLeaveSetting(guildId))],
      components: settingpanel.buildButtons(next, getGuildLeaveSetting(guildId)),
    });
  }
  ...
  if (interaction.isModalSubmit() && interaction.customId === "joinmsg_modal") {
    ...
    const next = setGuildJoinSetting(guildId, { ...setting, channelId, message });
...
async function handleLeaveMessagePanel(interaction) {
  ...
  if (interaction.isButton() && interaction.customId === "leavemsg_toggle") {
    ...
    const next = setGuildLeaveSetting(guildId, { ...setting, enabled: !setting.enabled });

    return interaction.update({
      embeds: [settingpanel.buildPanel(getGuildJoinSetting(guildId), next)],
      components: settingpanel.buildButtons(getGuildJoinSetting(guildId), next),
    });
  }
  ...
  if (interaction.isModalSubmit() && interaction.customId === "leavemsg_modal") {
    ...
    const next = setGuildLeaveSetting(guildId, { ...setting, channelId, message });
